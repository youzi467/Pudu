...
| 阶段 1 | OMR 黑盒集成（PDF/JPG→MusicXML） | ⬜ 未开始 | 落后（规划为阶段 2 前置，实际阶段 2 已用现成 MusicXML 绕过） |
| 阶段 2 | 五线谱→简谱（MVP v1） | ✅ 完成（已打 `phase-2`） | **超前**（规划文档状态仍为“未开始”，实际已 100% 校验） |
| 阶段 3 | 简谱→五线谱 | ⬜ 未开始 | 符合（本次计划对象） |
| 阶段 4 | AI / 深度学习 | ⬜ 未开始 | 符合 |
...
- 整体完成度（按工期权重）约 25–30%；完成阶段 3 后核心“双脑”闭环，产品即成“可用原型”。
...
- **不破坏阶段 2**：`jianpuToStaff` 为新增函数，对 `staffToJianpu` / 渲染器零侵入；单测须保证阶段 2 的 54/54 不回归。

---

## 6. 验收标准（Definition of Done）

- [ ] `jianpuToStaff` 实现并通过单测（覆盖 §5 反向九项边界）。
- [ ] `Score → MusicXML` 序列化可用，写出文件能被本仓库解析器读回且语义等价。
- [ ] Round-trip `staffToJianpu → jianpuToStaff` 对样本集音高序列 100% 守恒（阶段 2 单测 54/54 无回归）。
- [ ] CLI `--to-musicxml` 可演示端到端反向。
- [ ] 提交并打 `phase-3` 标签；README / research_report 阶段 3 状态更新。
