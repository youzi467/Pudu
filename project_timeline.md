...
| **Plan A 生产路径缺口（M2-opt-A2）** | gt 对齐法已修评测期泄漏；无 gt 真实推理的 a 小调泄漏已由修复版 `_apply_alters`（commit `997b3aa`）关闭 | **已完成** |
...
| **M2-opt-A** | Plan A 生产路径补全（M2-opt-A2） | **已完成（commit `997b3aa`）** | ~0.5 天 | 无 gt 真实推理 a 小调变化音不再误清零 | P2 |
...
> **推荐顺序修正（相对 07-15）**：阶段 3 与 OMR 已提前完成，故"下一步"不再是 M1/M2，而是 **先M0 救火（备份）→ 再 M2-opt（精度优化线，M2-opt-A2 已于 commit `997b3aa` 完成）**。M3(阶段4)/M4(阶段5) 顺序不变，F3 全量 A/B 已证实零效果、不作为瓶颈 gate。
...
| **P2** | **M2-opt-A2：Plan A 生产路径补全（已完成 commit `997b3aa`）** | 解除真实推理 a 小调变化音净负面 |
