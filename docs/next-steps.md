...
> [!NOTE]
> **（2026-08-06 更新）**：上条"`pitch_degree`（14%）仍是瓶颈"的触发前提已弱化——该 14% 数字经复核为评测标尺退化为随机配对的测量假象，oemer 真实音名准确率不可认证（独立复算 step 38.4% / step+octave 25.7%，另 LCS 估计 ~91.6% / ~55%，三法发散）。M2-opt-C 的启动应改以"R1 修标尺后的可信基线"为准，而非依赖旧的 14% 数字。详见 `docs/omr-engine-feasibility.md` 附录 A。
