...
> git 基线：本地 `HEAD=127838a`，`origin/main=0736c92`，**领先 2 提交待推送**（`a1ed452` 文档修正 + `127838a` P0 补丁固化）。

---

## 0. 当前位置（一句话）

核心双线（MusicXML⇄简谱双向转换 + OMR 黑盒集成）已打通并 MVP 达成；近期完成 P1（octave 波动证伪关闭）、全量文档审计、P0（oemer 补丁固化）、**P2 真实推理变音修复**（无 gt alter 推断，997b3aa）；剩余为 P3 F3 处置拍板、环境遗留修复、以及 P4 条件项。
...
| # | 模块 | 状态 | 关键指标 |
|---|---|---|---|
| 1 | 核心转换引擎（C++ 双向） | ✅ MVP | 117 gtest + 41 F3 Python 单测全绿；music21 跨语言 100% |
...
| 5 | F3 几何校正器 | ✅ 零效果 | 实验性、默认 OFF、不作上线 |
...
- **目标**：决定 F3 是「保留实验性标注」还是「移除」，并保证 CLI/文档一致。
- **背景**：F3 全量 A/B 证实对 oemer 0.1.8 **零效果**（OFF==ON 逐字节相同），已标为实验性、默认 OFF、不作音准改进上线。
- **选项**：
  - **A. 保留实验性**（推荐）：保留代码 + `--f3-geometric` 开关 + sidecar schema + 文档标注「零效果·实验性」；作为「我们验证过几何重算无收益」的记录，有简历/作品集价值。
  - **B. 移除**：删 `tools/geometric_pitch.py`、F3 相关单测、`docs/f3-*.md`、sidecar 导出；简化代码库。
- **完成标准**：拍板后 CLI `--help`、README、`docs/m2-real-run-guide.md` 描述与实际一致；若移除则 117 gtest + 41 单测调整后仍全绿。
- **依赖**：无。
- **预计**：0.5 天（保留）/ 1 天（移除）。
- **建议**：选 A（保留实验性）——零效果结论本身是工程价值，移除反而丢失「我们做过严谨 A/B」的证据。
