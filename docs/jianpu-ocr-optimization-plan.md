...
| **P1-2 预处理 A/B + 后处理前后对比** | ✅ 已落地 + 双轨真机完成（railA 干净 n=6 + railB 退化 n=36，2026-08-05） | `tools/omr_abtest_lib.py` + `omr_abtest_p1_2.py` + `omr_degrade_corpus.py`（轨 B，U3）已实现；团队质量关卡 252 全绿 + T04 不变量 24 全绿；**结论（回填 `docs/p1-2-abtest-design.md §12.5/§12.6/§12.7`）**：`preprocess_default=off`（轨 A 全 neutral；轨 B 退化图上 field 方向性负向 −19~−34pp，原"退化图靠预处理救"假设被证伪）、`postcorrect_default=off`（C1′ 13/13→43/13 干净 GT applied=0，Bug B 修复后稳定 PASS）、`confidence=directional-only`（合并 n=42 未达显著，SK-11 禁称"已验证"）；唯一低风险 opt-in 候选 `pre_low_contrast`，真实拍摄样本验证留 P2-2（U4） |
