...
| **单换引擎能否到 80%** | **不能直接回答——因为当前 13.6% 这个数字本身不可信**（见 §1）。修好标尺后再谈 |
| **最关键前置依赖** | **先修评测对齐算法，不是先换引擎**。约 1 人日，优先级高于本文所有其他建议 |
...
|---|---|---|---|
| `pitch_degree`（音名） | 13.56% | **~91.6%（上界，未独立复现，见附录 A）** | 真实值不可认证；待 R1 修标尺后重测，目前不能断言是否达标 |
...
**① 在音名（`pitch_degree`）口径上：方向乐观，但数字未证实。**
序列对齐估计 oemer 真实 step 准确率 ~91.6%（本文 LCS）或 ~38.4%（主理人 difflib 独立重算，见附录 A）——两者差距巨大，说明当前对齐方法不可信。修好标尺（R1）前**不能断言**是否已达 80%。若 R1 后真实值 >80%，则无需换引擎；但这属待验证，非已确定。
...
| # | 项目 | 等级 | 说明与建议 |
|---|---|---|---|
| **R1** | **评测标尺不可信** | 🔴 **最高** | §1 已论证。**不修此项，后续所有引擎对比都是无效实验。**必须先做 |
...
同时需修正一个流传已久的判断：原以为"音名（pitch_degree）是最弱短板、根因是 off-by-one 几何偏置"——这一判断已被推翻（`pitch_degree` 13.6% 是坏标尺的随机配对假象，816 个失败音符音级偏移近似均匀分布，非 ±1 集中；F3 几何校正器零效果也正因此被解释）。至于"真正短板是八度（step+octave）"——**这仍是待证假设**：本文 LCS 估计 step+octave ~55%，主理人独立重算仅 ~25.7%，标尺修好前无法定论。F3 零效果本身成立（靶子本不存在）。

### 6.2 建议的执行顺序

| 步骤 | 内容 | 工作量 | 产出 |
|---|---|---|---|
| **第一步（强烈建议）** | **修评测对齐**：把 `_merge_align` 的 positional fallback 换成 Needleman–Wunsch / DTW 保序对齐（以音高序列为锚，节奏错误不再破坏配对）；顺带修 GT Bug B 拍号；核查 p6 结构错位 | **~1 人日** | **oemer 的真实基线**。这一个数字决定后面所有决策 |
| **第二步** | 拍板验收口径：`field_pass` 还是 `pitch_degree`？MVP 单声部域还是 concerto 难域？ | 0.5 人日（决策） | 明确的 80% 定义 |
...
> **核心建议：先花 1 人日修标尺，再决定要不要花 2～4 周换引擎。**
> 这 1 人日可能直接告诉你"音名其实已经 90%，80% 目标在音名口径上已达成"，从而把整个项目的优化重心从"换引擎"转向"攻八度和节奏"——那是完全不同的一套打法。
...
### A.4 修订后的结论
1. **`pitch_degree` 13.6%（harness）不可信**：是 positional-fallback 随机配对的假象，不是 oemer 的音名准确率。
2. **"oemer 真实音名已 ~91.6%、真正短板是八度"未证实**：91.6% 不可复现；独立复算 step 仅 38.4%。"八度为真正短板"是待证假设。
3. **唯一稳健结论：评测标尺已坏（R1）**。在 `_merge_align` 改为 Needleman–Wunsch 全局对齐 + 替换罚分（~1 人日）之前，所有准确率数字（13.6% / 25.7% / 55% / 91.6%）都不可作为决策依据。
4. **F3 零效果结论仍然成立**，且"off-by-one 几何偏置"根因已被证伪（816 失败音符音级偏移近似均匀分布）——两者相互自洽。
5. **80% 验收决策被 R1 阻断**：必须先修标尺得到可信基线，再拍板口径（field_pass / pitch_degree / note_pass、MVP 单声部域 vs concerto 难域），最后才谈换引擎。
