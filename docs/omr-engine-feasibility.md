...
**这直接推翻了项目当前的核心假设。** 现有文档（`README.md:38`、`docs/system_design.md:7`）一致把 `pitch_degree`（音名）列为"最弱短板 / 优化靶心"，并据此设计了 F3 几何校正器（零效果）与 M2-opt-C 规则引擎（待做）。实际短板是**八度归属**（加线区判定、clef/track 归属）。
...
**建议口径**：以 **`field_pass` ≥ 80%** 作为上线线（字段级通过率，兼顾音高/时值/八度），并在 **MVP 单声部域**（单声部、≤2 升降号、无装饰音）而非 concerto 难域上验收。理由：`note_pass`（整音符全对）在 OMR 领域是极严苛指标，商业软件在真实扫描件上也难破 80%；concerto 是六页 Vivaldi 小提琴协奏曲，属难域，不应作为 MVP 验收基准。
